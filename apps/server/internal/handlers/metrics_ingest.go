...
func (a *MetricsAggregator) Add(name string, labels map[string]string, v float64) {
    a.mu.Lock()
    defer a.mu.Unlock()
    if _, ok := a.counter[name]; !ok { a.counter[name] = make(map[string]float64) }
...
func NewMetricsIngestHandler(agg *MetricsAggregator) *MetricsIngestHandler { return &MetricsIngestHandler{ agg: agg } }
...
func (h *MetricsIngestHandler) Ingest(c *gin.Context) {
    ...
    for _, m := range req.Metrics {
        if !allowedMetrics[m.Name] { continue }
        labels := map[string]string{}
        // 基础标签
        labels["source"] = req.Source
        if req.Tenant != "" { labels["tenant"] = req.Tenant }
        if req.SessionID != "" { labels["session"] = req.SessionID }
        for k, v := range m.Labels { labels[k] = v }
        val := m.Value
        if val == 0 { val = 1 }
