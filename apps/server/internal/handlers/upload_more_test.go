...
func buildMultipart(url, field, filename string, content []byte) (*http.Request, error) {
    ...
    if _, err := io.Copy(part, bytes.NewReader(content)); err != nil { return nil, err }
    _ = w.Close()
...
func newUploadRouterForTest(t *testing.T, cfg *config.Config) *gin.Engine {
    ...
    // 使用临时目录由 UploadHandler 自己创建（GetDefaultConfig 已设置 ./uploads），不在这里动磁盘
...
func TestUpload_EmptyFile_OK(t *testing.T) {
    ...
    w := httptest.NewRecorder()
    r.ServeHTTP(w, req)
    if w.Code != http.StatusOK { t.Fatalf("empty file should be accepted, got %d", w.Code) }
}
...
func TestUpload_MimePrefixMismatch_Reject(t *testing.T) {
    ...
    w := httptest.NewRecorder()
    r.ServeHTTP(w, req)
    if w.Code != http.StatusBadRequest { t.Fatalf("expected 400 for mime prefix mismatch, got %d", w.Code) }
}
