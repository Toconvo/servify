...
func TestEnhancedAI_CalculateConfidence(t *testing.T) {
	...
	if c1 <= c2 {
		t.Fatalf("expected weknora+docs confidence higher, got %v <= %v", c1, c2)
	}
}
...
func TestEnhancedAI_ConvertDocsToSources(t *testing.T) {
	...
	if len(ss) != 1 || ss[0].Title != "D" {
		t.Fatalf("unexpected sources: %+v", ss)
	}
}
...
func TestEnhancedAI_StatusAndCircuitBreaker(t *testing.T) {
	...
	if cb["failure_count"].(int) < 2 {
		t.Fatalf("expected failure_count >=2, got %v", cb["failure_count"])
	}
