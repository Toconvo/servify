...
func (s stubAI2) GetStatus(ctx context.Context) map[string]interface{} { return map[string]interface{}{"ok":true} }
...
func TestMessageRouter_RouteMessage_DBNil_FallbackAndContinue(t *testing.T) {
    ...
    case out := <-client.Send:
        if out.Type != "ai-response" { t.Fatalf("expected ai-response, got %s", out.Type) }
